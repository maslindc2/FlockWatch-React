...
describe("testing network", () => {
    ...
    test("should return expected structure when used", async () => {
        ...
        nock("http://flockwatch.io").get("/data/us-summary").reply(200, {
            data: {
        ...
        const {result} = renderHook(() => useUsSummaryData("http://flockwatch.io"), { wrapper });

        await vi.waitFor(() => {
            expect(result.current.isSuccess).toBe(true);
            expect(result.current.data?.data.totalBackyardFlocksNationwide).toBe(922);
            expect(result.current.data?.data.totalBirdsAffectedNationwide).toBe(174834748);
            expect(result.current.data?.data.totalCommercialFlocksNationwide).toBe(788);
            expect(result.current.data?.data.totalFlocksAffectedNationwide).toBe(1710);
