...
function App() {
    ...
    stateActiveSites.forEach((site: { production_type: string }) => {
        productionTypeCounts[site.production_type] =
            (productionTypeCounts[site.production_type] || 0) + 1;
    });
    const stateProductionTypes = Object.entries(productionTypeCounts)
        .map(([label, count]) => ({ label, count }))
        .sort((a, b) => b.count - a.count);

    const countyCounts: Record<
        string,
        { count: number; birds: number; types: Set<string> }
    > = {};
    stateActiveSites.forEach(
        (site: {
            county: string;
    ...
    function findSelectedStateColor(birdsAffectedInState: number): string {
        ...
        const t =
            Math.log(Math.max(1, birdsAffectedInState)) /
            Math.log(maxNumBirdsAffected);
            ...
            <main id="main-content" tabIndex={-1}>
                    ...
                    <div className="right-col-stack">
                        ...
                        <div className="chart-card">
                            <PieChart
                                ...
                                backyardFlocks={
                                    usSummaryAllTimeTotals.total_backyard_flocks_affected
                                }
                                commercialFlocks={
                                    usSummaryAllTimeTotals.total_commercial_flocks_affected
                                }
                            />
                        </div>
                        <div className="chart-card">
                ...
                <section className="chart-row">
                    <div className="full-width-wrapper">
                        <RecentConfirmations sites={sitesDataFromAPI.data} />
                    </div>
                </section>
                <section className="chart-row">
